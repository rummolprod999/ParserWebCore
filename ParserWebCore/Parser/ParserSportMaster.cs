...
namespace ParserWebCore.Parser
{
    public class ParserSportMaster: ParserAbstract, IParser
    {
        ...
        private protected void CreateTender(string url)
        {
            ...
            var attachText = htmlDoc.DocumentNode.SelectSingleNode("//td[. = 'Пакет документов']/following-sibling::td")?.InnerText?.Trim() ?? "";
            ...
            var tn = new TenderSportMaster("ООО «Спортмастер»", "http://zakupki.sportmaster.ru/", 216,
                new TypeSportmaster {PurName = purName, PurNum = purNum, DatePub = datePub, Href = url, DateEnd = dateEnd, Status = status, Attach = attach});
            ParserTender(tn);
        }
    }
}
...
{
    public class TenderAcron : TenderAbstract, ITender
    {
        private readonly TypeEtpu _tn;

        public TenderAcron(string etpName, string etpUrl, int typeFz, TypeEtpu tn) : base(etpName,
            etpUrl,
            typeFz)
        {
            _tn = tn;
        ...
        private void CreateLot(MySqlConnection connect, int idTender, int customerId, HtmlDocument htmlDoc)
        {
            ...
            var purObjects = htmlDoc.DocumentNode.SelectNodes(
                                 "//h2[. = 'Номенклатура продукции']/following-sibling::div[1]//table[contains(@class, 'contractor_search_table')]//tr[position() > 1]") ??
        ...
        private void CreatePurObjects(MySqlConnection connect, int customerId, HtmlNode purObject, int idLot)
        {
            ...
            var quant = quantT;
            var okei = (purObject.SelectSingleNode(".//td[4]")
                ?.InnerText ?? "").Trim();
            var insertLotitem =
                $"INSERT INTO {AppBuilder.Prefix}purchase_object SET id_lot = @id_lot, id_customer = @id_customer, name = @name, quantity_value = @quantity_value, okei = @okei, customer_quantity_value = @customer_quantity_value, price = @price, sum = @sum";
            var cmd20 = new MySqlCommand(insertLotitem, connect);
        ...
        private void AddAttachments(HtmlDocument htmlDoc, MySqlConnection connect, int idTender)
        {
            ...
            foreach (var doc in docs)
            {
                ...
                var urlAtt = $"https://etp.acron.ru{urlAttT}";

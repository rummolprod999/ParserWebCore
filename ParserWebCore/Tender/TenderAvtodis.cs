...
{
    public class TenderAvtodis : TenderAbstract, ITender
    {
        private readonly TypeAvtodis _tn;

        public TenderAvtodis(string etpName, string etpUrl, int typeFz, TypeAvtodis tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                var purName = nav.SelectSingleNode(
                                      "//h1")
                                  ?.Value?.Trim() ??
                              "";
                var insertLotitem =
                ...
                cmd20.Parameters.AddWithValue("@name", purName);
                ...
                var dt1 = nav.SelectSingleNode(
                                  "//td[. = 'Оплата']/following-sibling::td")
                              ?.Value?.Trim() ??
                          "";
                var dt2 = nav.SelectSingleNode(
                                  "//td[. = 'Дата загрузки']/following-sibling::td")
                              ?.Value?.Trim() ??
                          "";
                var dp = nav.SelectSingleNode(
                                 "//td[. = 'Куда']/following-sibling::td")
                             ?.Value?.Trim() ??
                         "";
                var dtt = $"Оплата: {dt1} Дата загрузки: {dt2}";
                if (!string.IsNullOrEmpty(dtt))
                {
                    ...
                    cmd16.Parameters.AddWithValue("@delivery_term", dtt);

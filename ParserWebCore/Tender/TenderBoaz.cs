...
{
    public class TenderBoaz : TenderAbstract, ITender
    {
        private readonly TypeBoaz _tn;
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                connect.Open();
                var selectTend =
                    $"SELECT id_tender FROM {AppBuilder.Prefix}tender WHERE purchase_number = @purchase_number AND doc_publish_date = @doc_publish_date AND type_fz = @type_fz";
                ...
                var customerId = 0;
                var organiserId = 0;
                var orgName = "АО «Организатор строительства Богучанского Алюминиевого Завода»";

    ...
    public class TenderComRzd : TenderAbstract, ITender
    {
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                connect.Open();
                var selectTend =
                    $"SELECT id_tender FROM {AppBuilder.Prefix}tender WHERE purchase_number = @purchase_number AND type_fz = @type_fz";
                ...
                var organiserId = 0;
                var pubDateT =
                    _driver.FindElement(By.XPath("//dt[. = 'Дата публикации процедуры']/following-sibling::dd"))?.Text
                        ?.Replace("Дата начала подачи заявок:", "")
                        .Trim().ReplaceHtmlEntyty().DelDoubleWhitespace().Trim() ??
                    throw new Exception("cannot find pubDateT");
                _tn.DatePub = pubDateT.ParseDateUn("dd.MM.yyyy");
                ...
                PlacingWay = _driver
                    .FindElementWithoutException(By.XPath("//dt[. = 'Тип процедуры']/following-sibling::dd"))
                    ?.Text.Trim() ?? "";
                ...
                var nmck = _driver
                    .FindElementWithoutException(
                        By.XPath("//dt[. = 'Начальная (максимальная) цена договора']/following-sibling::dd"))
                    ?.Text.Trim().ReplaceHtmlEntyty().ExtractPriceNew() ?? "";
                var insertLot =
                ...
                var cusName = _driver
                    .FindElementWithoutException(By.XPath("//dt[. = 'Заказчик']/following-sibling::dd"))
                    ?.Text.Trim() ?? "";
                ...
                var docs = _driver.FindElements(By.CssSelector("a.display-files__link"));
                GetDocs(docs, connect, idTender);
                var delivPlace = _driver
                    .FindElementWithoutException(By.XPath("//dt[. = 'Место проведения']/following-sibling::dd"))
                    ?.Text.Trim() ?? "";
                ...
                _driver.ExecutorJs(
                    "var elem = document.querySelectorAll('button[data-tab-open = \"2\"]'); elem[elem.length-1].click()");
                Thread.Sleep(1000);
                var lots = _driver.FindElements(By.XPath(
                    "//table[contains(.,'Наименование лота')]/tbody/tr[position() > 1]"));

...
{
    public class TenderFbRts : TenderAbstract, ITender
    {
        private readonly TypeFbRts _tn;

        public TenderFbRts(string etpName, string etpUrl, int typeFz, TypeFbRts tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                if (!string.IsNullOrEmpty(_tn.Region))
                {
                    ...
                    cmd16.Parameters.AddWithValue("@delivery_place", _tn.Region);
                    cmd16.Parameters.AddWithValue("@max_price", "");
                    cmd16.Parameters.AddWithValue("@delivery_term", "");
                    cmd16.ExecuteNonQuery();
                }

                foreach (var purObj in _tn.positions)
                {
                    var insertLotitem =
                    ...
                    cmd20.Parameters.AddWithValue("@quantity_value", purObj.Quantity);
                    cmd20.Parameters.AddWithValue("@okei", purObj.Okei);
                    cmd20.Parameters.AddWithValue("@customer_quantity_value", purObj.Quantity);
                    cmd20.Parameters.AddWithValue("@price", purObj.Price);
                    cmd20.Parameters.AddWithValue("@sum", purObj.Sum);

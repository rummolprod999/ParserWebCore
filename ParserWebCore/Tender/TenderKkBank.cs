...
{
    public class TenderKkBank : TenderAbstract, ITender
    {
        private readonly TypeKkbank _tn;

        public TenderKkBank(string etpName, string etpUrl, int typeFz, TypeKkbank tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            ...
            var dateEndT =
                (navigator.SelectSingleNode("//span[contains(., 'Дата окончания приёма заявок - ')]/b")
                    ?.Value ?? "").Trim();
            var (dateEndT1, dateEndT2) = dateEndT.GetTwoDataFromRegex(@"(\d{2}\.\d{2}\.\d{4}).+(\d{2}-\d{2})");
            var dateEnd = $"{dateEndT1} {dateEndT2}".ParseDateUn("dd.MM.yyyy HH-mm");
            ...
            using (var connect = ConnectToDb.GetDbConnection())
            {
                connect.Open();
                var selectTend =
                    $"SELECT id_tender FROM {Builder.Prefix}tender WHERE purchase_number = @purchase_number AND type_fz = @type_fz AND doc_publish_date = @doc_publish_date AND end_date = @end_date";
                ...
                PlacingWay =
                    (navigator.SelectSingleNode("//span[contains(., 'Вид конкурса – ')]/b")
                        ?.Value ?? "").Trim();
                ...
                cmd9.Parameters.AddWithValue("@scoring_date", _tn.ScoringDate);
                ...
                foreach (var doc in docs)
                {
                    ...
                    var urlAtt = $"https://kk.bank{urlAttT}";
                    if (!string.IsNullOrEmpty(fName))
                    {
                        ...
                        cmd10.Parameters.AddWithValue("@file_name", fName.DelDoubleWhitespace());
                ...
                var requirements = htmlDoc.DocumentNode.SelectNodes(
                                       "//h4[. = 'Требования к участникам конкурса']/following-sibling::ol/li") ??
                                   new HtmlNodeCollection(null);
                foreach (var req in requirements)
                {
                    var content = req.InnerText.DelDoubleWhitespace().Trim();
                    if (!string.IsNullOrEmpty(content))
                    {
                        var insertReq =
                            $"INSERT INTO {Builder.Prefix}requirement SET id_lot = @id_lot, content = @content, name = @name";
                        var cmd19 = new MySqlCommand(insertReq, connect);
                        cmd19.Prepare();
                        cmd19.Parameters.AddWithValue("@id_lot", idLot);
                        cmd19.Parameters.AddWithValue("@content", content);
                        cmd19.Parameters.AddWithValue("@name", "Требование к участникам конкурса");

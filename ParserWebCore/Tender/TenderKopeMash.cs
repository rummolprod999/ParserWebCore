...
{
    public class TenderKopeMash : TenderAbstract, ITender
    {
        private readonly TypeKopeMash _tn;

        public TenderKopeMash(string etpName, string etpUrl, int typeFz, TypeKopeMash tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                connect.Open();
                var selectTend =
                    $"SELECT id_tender FROM {AppBuilder.Prefix}tender WHERE purchase_number = @purchase_number AND type_fz = @type_fz AND doc_publish_date = @doc_publish_date";
                ...
                foreach (var doc in docs)
                {
                    ...
                    var fName = doc.SelectSingleNode("./parent::td/preceding-sibling::td")?.InnerText?.Trim();
                    var urlAtt = $"https://www.kopemash.ru{urlAttT}";

...
{
    public class TenderKorabel : TenderAbstract, ITender
    {
        private readonly TypeKorabel _tn;

        public TenderKorabel(string etpName, string etpUrl, int typeFz, TypeKorabel tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                var organiserId = 0;
                var orgName = EtpName;
                PlacingWay = (document.QuerySelector("td:contains(''Срочность заявки) + td")?.TextContent ?? "").Trim();
                ...
                if (!string.IsNullOrEmpty(_tn.DelivTerm))
                {
                    ...
                    cmd16.Parameters.AddWithValue("@delivery_term", $"Сроки исполнения: {_tn.DelivTerm}");
                    cmd16.ExecuteNonQuery();
                }

                var purObjects =
                    document.QuerySelectorAll("table.tenders_cats tr[style = 'background-color: #f6f6f6;']");
                foreach (var purObject in purObjects)
                {
                    var namePo = (purObject.QuerySelector("td:nth-child(2) b")?.TextContent ?? "").Trim();
                    var quantT = (purObject.QuerySelector("td:nth-child(3)")?.TextContent ?? "").Trim();
                    var quant = quantT.GetDataFromRegex(@"}\s*(\d+)");
                    quant = quant == "" ? quantT : quant;
                    var insertLotitem =

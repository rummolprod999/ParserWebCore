...
{
    public class TenderMetal100 : TenderAbstract, ITender
    {
        private readonly ChromeDriver _driver;
        private readonly TypeMetal100 _tn;

        public TenderMetal100(string etpName, string etpUrl, int typeFz, TypeMetal100 tn, ChromeDriver driver) : base(
            etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                var nmck = _driver
                    .FindElementWithoutException(By.XPath("//span[contains(., 'сумма контракта')]"))
                    ?.Text?.Trim() ?? "";
                if (!string.IsNullOrEmpty(nmck))
                {
                ...
                if (!string.IsNullOrEmpty(delivTPlace) || !string.IsNullOrEmpty(delivTerm))
                {
                    ...
                    cmd16.Parameters.AddWithValue("@delivery_place", delivTPlace);
                ...
                var lots = _driver.FindElements(By.XPath(
                    "//table[@class = 'orderTable no-margin']//tr[position() > 1]"));

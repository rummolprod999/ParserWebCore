...
{
    public class TenderMobWin : TenderAbstract, ITender
    {
        private readonly TypeMobWin _tn;

        public TenderMobWin(string etpName, string etpUrl, int typeFz, TypeMobWin tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                connect.Open();
                var selectTend =
                    $"SELECT id_tender FROM {AppBuilder.Prefix}tender WHERE purchase_number = @purchase_number AND end_date = @end_date AND type_fz = @type_fz";
                ...
                foreach (var dd in _tn.Attachments)
                {
                    if (string.IsNullOrEmpty(dd.Name)) continue;
                    var insertAttach =
                    ...
                    cmd10.Parameters.AddWithValue("@file_name", dd.Name);
                    cmd10.Parameters.AddWithValue("@url", dd.Url);

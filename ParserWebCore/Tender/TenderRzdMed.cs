    ...
    public class TenderRzdMed : TenderAbstract, ITender
    {
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                var region = ((string)_tn.token.SelectToken("regions.name") ?? "").Trim();
                var idRegion = GetRegionFromString(region, connect);
                var insertTender =
                ...
                var cusName = ((string)_tn.token.SelectToken("company.legal_detail.name") ?? "").Trim();
                if (!string.IsNullOrEmpty(cusName))
                {
                    ...
                    {
                        reader7.Close();
                        var inn = ((string)_tn.token.SelectToken("company.legal_detail.inn") ?? "").Trim();
                        var kpp = ((string)_tn.token.SelectToken("company.legal_detail.kpp") ?? "").Trim();
                        var insertCustomer =
                ...
                var lotN = ((string)_tn.token.SelectToken("lots[0].id") ?? "").Trim();
                var attacments = GetElements(_tn.token, "files");
                WriteAttachments(connect, attacments, idTender, lotN);
                var lotNum = 1;
                var lots = GetElements(_tn.token, "lots");
                lots.ForEach(l =>
                {
                    var maxPrice = ((string)l.SelectToken("average_price") ?? "").Trim();
                    var curr = ((string)_tn.token.SelectToken("currency.name") ?? "").Trim();
                    ;
                    var insertLot =
                    ...
                    cmd18.Parameters.AddWithValue("@max_price", maxPrice);
                    cmd18.Parameters.AddWithValue("@currency", curr);
                    cmd18.Parameters.AddWithValue("@finance_source", "");
                    cmd18.ExecuteNonQuery();
                    var idLot = (int)cmd18.LastInsertedId;
                    lotNum++;
                    var pObs = GetElements(l, "items");
                    pObs.ForEach(p =>
                    {
                        var name = ((string)p.SelectToken("name") ?? "").Trim();
                        var quant = ((string)p.SelectToken("amount") ?? "").Trim();
                        var okei = ((string)p.SelectToken("unit.name") ?? "").Trim();
                        var price = ((string)p.SelectToken("average_price") ?? "").Trim();
                        var insertLotitem =
                        ...
                        cmd20.Parameters.AddWithValue("@price", price);
        ...
        private void WriteAttachments(MySqlConnection connect, List<JToken> attachments, int idTender, string lotN)
        {
            foreach (var att in attachments)
            {
                ...
                var url = $"https://zakupki.rzd-medicine.ru/api/purchase/order/lot/{lotN}/file/{webPath}";

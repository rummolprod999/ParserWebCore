...
{
    public class TenderSamCom : TenderAbstract, ITender
    {
        private readonly TypeSamCom _tn;

        public TenderSamCom(string etpName, string etpUrl, int typeFz, TypeSamCom tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                if (!string.IsNullOrEmpty(orgName))
                {
                    ...
                    {
                        var phone = "+7(846) 336-14-02";
                        var email = "info@samcomsys.ru";
                        var inn = "";
                        var kpp = "";
                        var contactPerson = "";
                        var addOrganizer =
                ...
                foreach (var att in _tn.Attachments)
                {
                    var insertAttach =
                    ...
                    cmd10.Parameters.AddWithValue("@file_name", att.Name);

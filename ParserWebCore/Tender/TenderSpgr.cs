...
{
    public class TenderSpgr : TenderAbstract, ITender
    {
        private readonly TypeSpgr _tn;

        public TenderSpgr(string etpName, string etpUrl, int typeFz, TypeSpgr tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        private void AddLots(HtmlDocument htmlDoc, MySqlConnection connect, int idTender, int customerId)
        {
            ...
            var poList =
                htmlDoc.DocumentNode.SelectNodes(
                    "//table[@class = 't_lot_table']//tr[@class]") ??
            ...
            if (poList.Count != 0)
            {
                foreach (var pp in poList)
                {
                    var namePo = (pp.SelectSingleNode(".//td[2]")
                        ?.InnerText ?? "").Trim();
                    namePo = $"{namePo}".Trim();
                    var namePoT = namePo.Replace("&nbsp;", " ").DelDoubleWhitespace();
                    var name = namePoT.GetDataFromRegex(@"Наименование товара:\s+(.+)\s+Кол-во");
                    if (!string.IsNullOrEmpty(name))
                    {
                        namePo = name;
                    }

                    var okeiP = namePoT.GetDataFromRegex(@"Кол-во\s+:\s+[\d.,]+\s+([^\s]+)(?=Доп.хар-ки:)*");
                    ;
                    var quantityP = namePoT.GetDataFromRegex(@"Кол-во\s+:\s+([\d.,]+)");
                    var insertLotitem =

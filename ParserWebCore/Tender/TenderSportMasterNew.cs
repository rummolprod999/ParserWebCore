...
{
    public class TenderSportMasterNew : TenderAbstract, ITender
    {
        private readonly TypeSport1 _tn;

        public TenderSportMasterNew(string etpName, string etpUrl, int typeFz, TypeSport1 tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                docString = docString.GetDataFromRegex(@"(?s)window.jsParams\s+=\s+({""tenderId"".+});\s+</script>")
                    .Trim();
                ...
                var notice = ((string) jObj.SelectToken("stage.note") ?? "").Trim();
                ...
                cmd9.Parameters.AddWithValue("@notice_version", notice);
                ...
                var attachments = GetElements(jObj, "stage.arFiles");
                foreach (var att in attachments)
                {
                    var name = ((string) att.SelectToken("name") ?? "").Trim();
                    var webPath = ((string) att.SelectToken("src") ?? "").Trim();
                    var urlAtt = $"https://zakupki.sportmaster.ru{webPath}";
                ...
                var lotNum = 1;
                var currency = ((string) jObj.SelectToken("stage.currency") ?? "").Trim();
                var insertLot =

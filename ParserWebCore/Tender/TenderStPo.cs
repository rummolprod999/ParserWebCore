...
{
    public class TenderStPo : TenderAbstract, ITender
    {
        private readonly TypeStPo _tn;

        public TenderStPo(string etpName, string etpUrl, int typeFz, TypeStPo tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                var cat = nav.SelectSingleNode(
                                  "//li[contains(., 'Дата окончания работ:')]")
                ...
                if (!string.IsNullOrEmpty(cat))
                {
                    ...
                    cmd16.Parameters.AddWithValue("@delivery_term", cat.ReplaceHtmlEntyty().Trim());
                    cmd16.ExecuteNonQuery();
                }

                var pName = nav.SelectSingleNode(
                                    "//div[@class = 'tender_text']")
                                ?.Value?.Trim().DelDoubleWhitespace().Trim() ??
                            "";
                ...
                cmd20.Parameters.AddWithValue("@name", pName);

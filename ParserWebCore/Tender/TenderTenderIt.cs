...
{
    public class TenderTenderIt : TenderAbstract, ITender
    {
        private readonly TypeTenderIt _tn;

        public TenderTenderIt(string etpName, string etpUrl, int typeFz, TypeTenderIt tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                var status = navigator.SelectSingleNode(
                        "//span[contains(@class, 'tender-status')]")
                    ?.Value?.Trim() ?? "";
                var insertTender =
                ...
                var price = (navigator.SelectSingleNode("//li[contains(., 'Общий бюджет:')]")
                ...
                if (!string.IsNullOrEmpty(_tn.City))
                {
                    ...
                    cmd16.Parameters.AddWithValue("@delivery_place", _tn.City);

    ...
    public class TenderTpta : TenderAbstract, ITender
    {
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                var idTender = AddTender(connect, organiserId, idPlacingWay, idEtp, cancelStatus, dateUpd, printForm,
                    updated);
                customerId = AddCustomer(orgName, connect, customerId);
        ...
        {
            foreach (var typeObjectTpta in _tn.ObjectsPurchase)
            {
                var insertLotitem =
                ...
                cmd20.Parameters.AddWithValue("@name", typeObjectTpta.Name);
                cmd20.Parameters.AddWithValue("@quantity_value", typeObjectTpta.Quantity);
                cmd20.Parameters.AddWithValue("@okei", typeObjectTpta.Okei);
                cmd20.Parameters.AddWithValue("@customer_quantity_value", typeObjectTpta.Quantity);
        ...
        private static int AddLot(MySqlConnection connect, int idTender)
        {
            var lotNum = 1;
            var insertLot =
        ...
        private int AddTender(MySqlConnection connect, int organiserId, int idPlacingWay, int idEtp, int cancelStatus,
            DateTime dateUpd, string printForm, bool updated)
        {
            var insertTender =

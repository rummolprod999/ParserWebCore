...
{
    public class TenderUdsOil : TenderAbstract, ITender
    {
        private readonly TypeUdsOil _tn;

        public TenderUdsOil(string etpName, string etpUrl, int typeFz, TypeUdsOil tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                if (!string.IsNullOrEmpty(_tn.Href))
                {
                    ...
                    cmd10.Parameters.AddWithValue("@file_name", "Тендерная документация");

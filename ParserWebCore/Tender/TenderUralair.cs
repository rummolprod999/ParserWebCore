...
{
    public class TenderUralair : TenderAbstract, ITender
    {
        private readonly TypeUralair _tn;

        public TenderUralair(string etpName, string etpUrl, int typeFz, TypeUralair tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                _tn.Attachments.ForEach(x =>
                {
                    if (x.Name.Contains("Извещение"))
                    {
                        _tn.Href = x.Url;
                    }
                });

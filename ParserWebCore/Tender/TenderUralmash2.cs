...
{
    public class TenderUralmash2 : TenderAbstract, ITender
    {
        private readonly TypeUral2 _tn;

        public TenderUralmash2(string etpName, string etpUrl, int typeFz, TypeUral2 tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                connect.Open();
                var selectTend =
                    $"SELECT id_tender FROM {Builder.Prefix}tender WHERE purchase_number = @purchase_number AND type_fz = @type_fz AND end_date = @end_date";

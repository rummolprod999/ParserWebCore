...
{
    public class TenderYangPur : TenderAbstract, ITender
    {
        private readonly TypeYangPur _tn;

        public TenderYangPur(string etpName, string etpUrl, int typeFz, TypeYangPur tn) : base(etpName, etpUrl,
            typeFz)
        {
            _tn = tn;
        }
        ...
        public void ParsingTender()
        {
            using (var connect = ConnectToDb.GetDbConnection())
            {
                ...
                foreach (var att in _tn.Attachments)
                {
                    ...
                    cmd10.Parameters.AddWithValue("@file_name", att.Item2);
                    cmd10.Parameters.AddWithValue("@url", att.Item1);
